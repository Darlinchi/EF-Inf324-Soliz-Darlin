...
<body>
    <div class="container mt-5">
        ...
        <form action="{{ route('pedidos.update', $pedido->id) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <select class="form-control" id="usuario_id" name="usuario_id" required>
                    ...
                    @foreach ($usuarios as $usuario)
                        <option value="{{ $usuario->id }}" {{ $usuario->id == $pedido->usuario_id ? 'selected' : '' }}>
                            {{ $usuario->nombre }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <input type="number" class="form-control" id="cantidad" name="cantidad" value="{{ $pedido->cantidad }}" required min="1">
            </div>

            <!-- Campo de total (deshabilitado para ser calculado automáticamente) -->
            <div class="mb-3">
                <label for="total" class="form-label">Total</label>
                <input type="number" class="form-control" id="total" name="total" value="{{ $pedido->total }}" required min="0" step="any" readonly>
            </div>
    ...
    <script>
        ...
        // Calcular el total al cargar la página (para mostrar el total inicial)
        calcularTotal();
    </script>
</body>
</html>
